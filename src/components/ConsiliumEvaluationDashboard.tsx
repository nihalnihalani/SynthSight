// components/ConsiliumEvaluationDashboard.tsx
...
import { ConsiliumEvaluationResult, ExpertEvaluation, LLMProvider } from '../services/consiliumEvaluationService';

interface ConsiliumEvaluationDashboardProps {
  evaluationResult: ConsiliumEvaluationResult;
  onClose?: () => void;
  onRetry?: () => void;
}

const ConsiliumEvaluationDashboard: React.FC<ConsiliumEvaluationDashboardProps> = ({
  evaluationResult,
  onClose,
  onRetry
}) => {
  const [activeTab, setActiveTab] = useState<'overview' | 'experts' | 'research' | 'recommendations' | 'settings'>('overview');
  const [selectedExpert, setSelectedExpert] = useState<ExpertEvaluation | null>(null);
  ...
  const getProviderIcon = (provider: string) => {
    ...
    if (provider.includes('Gemini')) return <Zap className="w-5 h-5 text-blue-600" />;
    if (provider.includes('Mistral')) return <Shield className="w-5 h-5 text-purple-600" />;
    return <Brain className="w-5 h-5 text-gray-600" />;
  };

  const getResearchIcon = (source: string) => {
    switch (source.toLowerCase()) {
      case 'web search': return <Globe className="w-4 h-4 text-blue-500" />;
      case 'wikipedia': return <BookOpen className="w-4 h-4 text-gray-500" />;
      case 'arxiv': return <FileText className="w-4 h-4 text-orange-500" />;
      case 'github': return <Code className="w-4 h-4 text-gray-800" />;
      case 'sec': return <DollarSign className="w-4 h-4 text-green-500" />;
      ...
      >
        ...
        <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white p-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="text-2xl font-bold">Consilium Multi-LLM Evaluation</h2>
                <p className="text-blue-100">Real-time AI consensus with research integration</p>
              </div>
            </div>
            <div className="flex items-center space-x-2">
              {onRetry && (
                <button
                  ...
                  className="flex items-center space-x-2 px-3 py-2 bg-white bg-opacity-20 hover:bg-opacity-30 rounded-lg transition-colors disabled:opacity-50"
                >
        ...
        <div className="border-b border-gray-200">
          <nav className="flex space-x-8 px-6">
            {[
              ...
              { id: 'settings', label: 'LLM Providers', icon: Settings }
            ].map((tab) => (
              <button
                key={tab.id}
        ...
        <div className="p-6 overflow-y-auto h-full">
          {activeTab === 'overview' && (
            <div className="space-y-6">
              ...
              <div className="bg-white border border-gray-200 rounded-lg p-6">
                <h3 className="text-lg font-semibold text-gray-900 mb-4">Multi-LLM Expert Evaluation Scores</h3>
                    ...
                    >
                      <div className="flex items-center space-x-3">
                        <div className="w-10 h-10 bg-blue-100 rounded-full flex items-center justify-center">
                          {getProviderIcon(expert.provider)}
                        </div>
                        <div>
                          <p className="font-medium text-gray-900">{expert.expertName}</p>
                          <p className="text-sm text-gray-600">{expert.role.replace('_', ' ')} • {expert.provider}</p>
                        </div>
                      </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="font-medium">{evaluationResult.evaluationMetadata.llmProviders.length}</p>
                    </div>
                  </div>
                </div>
              </div>
                ...
                {evaluationResult.expertEvaluations.map((expert) => (
                  ...
                  >
                    <div className="text-center">
                      ...
                      <p className="text-xs text-gray-500">{expert.provider}</p>
                    </div>
                  </button>
                ))}
              </div>
              ...
              {selectedExpert && (
                ...
                >
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center space-x-2">
                      {getProviderIcon(selectedExpert.provider)}
                      <span className="text-sm text-gray-600">{selectedExpert.provider}</span>
                    </div>
                  </div>
                  ...
                  {/* Research Used */}
                  <div className="mb-6">
                    <h4 className="text-lg font-medium text-gray-900 mb-2">Research Sources Used</h4>
                    <div className="flex flex-wrap gap-2">
                      {selectedExpert.researchUsed.map((source, index) => (
                        <div key={index} className="flex items-center space-x-1 px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-sm">
                          {getResearchIcon(source)}
                          <span>{source}</span>
                        </div>
                      ))}
                    </div>
                ...
                <div className="flex items-center space-x-4 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-gray-600">
                      ...
                      <span className="ml-2">Confidence: {evaluationResult.finalRecommendation.confidence}/10</span>
                    </p>
                  </div>
                </div>
          ...
          {activeTab === 'settings' && (
            <div className="space-y-6">
              <div className="bg-white border border-gray-200 rounded-lg p-6">
                <h3 className="text-lg font-semibold text-gray-900 mb-4">LLM Provider Status</h3>
                
                <div className="space-y-4">
                  {evaluationResult.evaluationMetadata.llmProviders.map((provider, index) => (
                    <div key={index} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                      <div className="flex items-center space-x-3">
                        {getProviderIcon(provider)}
                        <div>
                          <p className="font-medium text-gray-900">{provider}</p>
                          <p className="text-sm text-gray-600">Used in evaluation</p>
                        </div>
                      </div>
                      <div className="flex items-center space-x-2">
                ...
                <div className="mt-6 p-4 bg-blue-50 rounded-lg">
                  <h4 className="text-md font-medium text-blue-900 mb-2">Consilium MCP Integration</h4>
                  <p className="text-blue-800 text-sm">
                    This evaluation uses real Consilium MCP functions including web search, Wikipedia, arXiv, GitHub, and SEC research capabilities.
                    All LLM providers have access to these research functions for enhanced evaluation quality.
                  </p>
                </div>
              </div>
            </div>
...
export default ConsiliumEvaluationDashboard;
